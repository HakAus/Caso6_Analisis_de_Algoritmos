...
Flock.prototype.set_view_mode = function(p_view_mode) {
	for (var i = 0; i < this.flock_size; i++) {
		// var bird = this.birds[i];
		this.birds[i].set_view_mode(p_view_mode);
	}
};

Flock.prototype.set_view_option = function(view_option) {
	for (var i = 0; i < this.flock_size; i++) {
		var bird = this.birds[i];
		bird.set_view_option(view_option);
	}
};
