var Bird = function(id, scene, octree, obstacles, velocity_hash, position, orientation) {
	this.id = id;
	this.scene = scene;
	this.octree = octree;
	...
	// Structures for bird data collection
	this.data = [];
...
Bird.prototype.get_data = function() {
	var buffer = "";
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	// this.distance_travelled += this.previous_position.distanceTo(displacement);
	var distance= this.previous_position.distanceTo(displacement);
	var info = "{" + this.id + "," + distance + "," + this.velocity.length() + "},";
	this.data.push(info);
